...
// MARK: - Private Models (V2)

private enum CrosswordV2Direction {
    case across, down
}

private struct CrosswordV2Clue: Identifiable {
    let id = UUID()
    let number: Int
    let direction: CrosswordV2Direction
    let text: String
    let startRow: Int
    let startCol: Int
    let length: Int
...
private struct CrosswordV2PuzzleData {
    let title: String
    let solution: [[Character]]
    let clues: [CrosswordV2Clue]
}

// MARK: - Puzzle Definitions

private func makeV2Puzzles() -> [CrosswordV2PuzzleData] {
    let sol1: [[Character]] = [
        ["S","W","I","F","T",".","."],
        ["T",".","H",".","R","A","P"],
        ["A",".","L",".","A","P","P"],
    ...
    let clues1: [CrosswordV2Clue] = [
        CrosswordV2Clue(number: 1, direction: .across, text: "Apple's programming language", startRow: 0, startCol: 0, length: 5),
        CrosswordV2Clue(number: 6, direction: .across, text: "Code + Edit (abbr)", startRow: 3, startCol: 1, length: 4),
        CrosswordV2Clue(number: 7, direction: .across, text: "Collection of ordered elements", startRow: 5, startCol: 1, length: 5),
        CrosswordV2Clue(number: 1, direction: .down, text: "Pile metaphor in CS", startRow: 0, startCol: 0, length: 5),
        CrosswordV2Clue(number: 2, direction: .down, text: "Loop keyword in Swift", startRow: 0, startCol: 2, length: 5),
        CrosswordV2Clue(number: 3, direction: .down, text: "View layout container", startRow: 0, startCol: 4, length: 5),
        CrosswordV2Clue(number: 4, direction: .down, text: "Fruit company", startRow: 1, startCol: 5, length: 4),
        CrosswordV2Clue(number: 5, direction: .down, text: "SwiftUI modifier target", startRow: 1, startCol: 6, length: 4),
        CrosswordV2Clue(number: 8, direction: .down, text: "Map function result", startRow: 4, startCol: 4, length: 3)
    ]
    ...
    let clues2: [CrosswordV2Clue] = [
        CrosswordV2Clue(number: 1, direction: .across, text: "Remote storage service", startRow: 0, startCol: 1, length: 5),
        CrosswordV2Clue(number: 3, direction: .across, text: "Apple's IDE", startRow: 1, startCol: 0, length: 5),
        CrosswordV2Clue(number: 5, direction: .across, text: "SwiftUI building block", startRow: 4, startCol: 0, length: 4),
        CrosswordV2Clue(number: 1, direction: .down, text: "Dark theme for Xcode", startRow: 0, startCol: 1, length: 5),
        CrosswordV2Clue(number: 2, direction: .down, text: "Binary digit pair", startRow: 1, startCol: 0, length: 1),
        CrosswordV2Clue(number: 4, direction: .down, text: "Software defect", startRow: 0, startCol: 4, length: 5)
    ]
    ...
    let clues3: [CrosswordV2Clue] = [
        CrosswordV2Clue(number: 1, direction: .across, text: "SwiftUI data connection", startRow: 0, startCol: 0, length: 4),
        CrosswordV2Clue(number: 4, direction: .across, text: "Graph elements", startRow: 3, startCol: 0, length: 5),
        CrosswordV2Clue(number: 5, direction: .across, text: "Color tint property", startRow: 5, startCol: 0, length: 3),
        CrosswordV2Clue(number: 1, direction: .down, text: "SwiftUI tree component", startRow: 0, startCol: 0, length: 5),
        CrosswordV2Clue(number: 2, direction: .down, text: "Metadata annotation", startRow: 0, startCol: 3, length: 4),
        CrosswordV2Clue(number: 3, direction: .down, text: "Graph connection node", startRow: 3, startCol: 2, length: 1)
    ]

    return [
        CrosswordV2PuzzleData(title: "Swift & iOS", solution: sol1, clues: clues1),
        CrosswordV2PuzzleData(title: "Xcode World", solution: sol2, clues: clues2),
        CrosswordV2PuzzleData(title: "Data & Bind", solution: sol3, clues: clues3)
    ]
}

// MARK: - ViewModel

private class CrosswordV2ViewModel: ObservableObject {
    let puzzles: [CrosswordV2PuzzleData]
    @Published var currentPuzzleIndex: Int = 0
    @Published var userGrid: [[Character?]]
    @Published var selectedCell: (row: Int, col: Int)? = nil
    @Published var direction: CrosswordV2Direction = .across
    @Published var cellStates: [[V2CellState]]
    @Published var solved: Bool = false
    @Published var showMessage: String = ""

    enum V2CellState { case none, correct, wrong }

    init() {
        self.puzzles = makeV2Puzzles()
        let size = 7
    ...
    var currentPuzzle: CrosswordV2PuzzleData { puzzles[currentPuzzleIndex] }
    ...
    func activeClue() -> CrosswordV2Clue? {
...
struct CrosswordViewV2: View {
    @StateObject private var vm = CrosswordV2ViewModel()
    ...
    private func glassCellBg(isSelected: Bool, inWord: Bool, state: CrosswordV2ViewModel.V2CellState) -> some View {
        Group {
            if isSelected {
    ...
    private func glassLetterColor(_ state: CrosswordV2ViewModel.V2CellState) -> Color {
        switch state {
    ...
    private func activeClueCard(_ clue: CrosswordV2Clue) -> some View {
        HStack(spacing: 8) {
    ...
    private func glassClueSection(title: String, clues: [CrosswordV2Clue]) -> some View {
