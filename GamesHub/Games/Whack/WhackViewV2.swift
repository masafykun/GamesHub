...
struct WhackViewV2: View {
    ...
    // Miss tracking (tap on inactive hole)
    @State private var lastMissIndex: Int? = nil
                        ...
                        Button { handleTap(index: mole.id) } label: {
                            ZStack {
                                Ellipse()
                                    ...
                                    .frame(height: 55).padding(.top, 28)
                                if mole.isActive {
                                    WhackMoleCharacter()
    ...
    private var headerView: some View {
        HStack(spacing: 0) {
            ...
            // Timer
            WhackStatCard(label: "Time", value: "\(timeRemaining)s", icon: "clock.fill", color: timeRemaining <= 5 ? .red : .cyan)
        }
        .padding(.horizontal, 20)
        .padding(.vertical, 12)
    ...
    private func startGame() {
        ...
        // Start countdown timer (1s intervals)
        ...
        // Start mole popping timer
        scheduleMolePop()
    }

    private func scheduleMolePop() {
        moleTimer?.invalidate()
        // Pop interval: slightly shorter than visible duration so overlap is possible
    ...
    private func popRandomMoles() {
        // Choose 1 or 2 inactive holes randomly
        ...
        let shuffled = inactiveIndices.shuffled().prefix(count)

        for idx in shuffled {
            moles[idx].isActive = true

            // Auto-hide after visible duration
            let hideTimer = Timer.scheduledTimer(withTimeInterval: moleVisibleDuration, repeats: false) { _ in
                if moles[idx].isActive {
                    // Mole escaped — no penalty (penalty only on explicit miss tap)
            ...
            activeMoleTimers[idx] = hideTimer
        }
    }
    ...
    private func endGame() {
        ...
        for i in moles.indices {
            moles[i].isActive = false
        }

        // Record score, keep last 5
    ...
    private func adjustDifficulty() {
        ...
        if avg >= 100 {
            ...
            // Steady — small nudge
            moleVisibleDuration = min(1.2, max(0.8, moleVisibleDuration))
        }
