...
private struct CRV2Cell {
    var owner: CRV2Player = .none
    var atoms: Int = 0
...
private class CRV2GameModel: ObservableObject {
    static let rows = 6
    static let cols = 6

    @Published var grid: [[CRV2Cell]] = Array(
        repeating: Array(repeating: CRV2Cell(), count: cols),
        count: rows
    )
    @Published var currentPlayer: CRV2Player = .red
    @Published var gameOver: Bool = false
    @Published var winner: CRV2Player = .none
    ...
    func aiMove() {
        ...
        let explosive = validCells.filter { r, c in
            let mass = grid[r][c].criticalMass(row: r, col: c,
                                                rows: CRV2GameModel.rows, cols: CRV2GameModel.cols)
    ...
    private func allCells() -> [(Int, Int)] {
        var result: [(Int, Int)] = []
        for r in 0..<CRV2GameModel.rows {
            for c in 0..<CRV2GameModel.cols { result.append((r, c)) }
        }
        return result
    }
    ...
    private func getNeighbors(row: Int, col: Int) -> [(Int, Int)] {
        ...
        if row < CRV2GameModel.rows - 1 { result.append((row + 1, col)) }
        if col > 0        { result.append((row, col - 1)) }
        if col < CRV2GameModel.cols - 1 { result.append((row, col + 1)) }
        return result
    }
    ...
    func reset() {
        grid = Array(repeating: Array(repeating: CRV2Cell(), count: CRV2GameModel.cols),
                     count: CRV2GameModel.rows)
        currentPlayer = .red
        gameOver = false
        winner = .none
        isAIThinking = false
...
struct ChainReactionViewV2: View {
    @StateObject private var model = CRV2GameModel()
    ...
    private var gridGlass: some View {
        GeometryReader { geo in
            ...
            let cellSize = (side - 12) / CGFloat(CRV2GameModel.cols)

            VStack(spacing: 2) {
                ForEach(0..<CRV2GameModel.rows, id: \.self) { row in
                    HStack(spacing: 2) {
                        ForEach(0..<CRV2GameModel.cols, id: \.self) { col in
                            CRV2CellView(
    ...
    private func scoreCard(player: CRV2Player, label: String) -> some View {
        let cells = (0..<CRV2GameModel.rows).flatMap { r in
            (0..<CRV2GameModel.cols).filter { c in model.grid[r][c].owner == player }
        }.count
        let atoms = (0..<CRV2GameModel.rows).flatMap { r in
            (0..<CRV2GameModel.cols).compactMap { c -> Int? in
...
private struct CRV2CellView: View {
    let cell: CRV2Cell
    ...
    var body: some View {
        ZStack {
            ...
            crV2AtomDots(count: cell.atoms, color: cell.owner.color,
...
private func crV2AtomDots(count: Int, color: Color, glow: Color, size: CGFloat) -> some View {
