...
// MARK: - Models
enum NMrgV2GamePhase { case start, playing, gameOver }
...
struct NMrgV2Board {
    ...
    mutating func swipe(_ dir: NMrgV2Direction) -> Bool {
        switch dir {
...
enum NMrgV2Direction { case left, right, up, down }

func nmrgV2TileColor(_ value: Int) -> Color {
    switch value {
...
// MARK: - Main View
struct NumberMergeViewV2: View {
    @State private var board = NMrgV2Board()
    @State private var phase: NMrgV2GamePhase = .start
                    ...
                    ForEach(0..<5, id: \.self) { c in
                        ...
                        ZStack {
                            RoundedRectangle(cornerRadius: 10)
                                .fill(val == 0 ? Color.white.opacity(0.05) : nmrgV2TileColor(val))
    ...
    func handleSwipe(_ value: DragGesture.Value) {
        ...
        let dir: NMrgV2Direction = abs(dx) > abs(dy) ? (dx > 0 ? .right : .left) : (dy > 0 ? .down : .up)
