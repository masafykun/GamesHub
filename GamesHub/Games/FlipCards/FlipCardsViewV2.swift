...
struct FlCdV2Card: Identifiable {
    let id: Int
    let value: Int
...
struct FlipCardsViewV2: View {
    @State private var cards: [FlCdV2Card] = []
    @State private var phase: FlCdV2Phase = .start
    ...
    func startGame() {
        ...
        cards = values.enumerated().map { FlCdV2Card(id: $0.offset, value: $0.element) }
        moves = 0
        firstIndex = nil
        isLocked = false
    ...
    func handleTap(card: FlCdV2Card) {
        guard !isLocked,
              !card.isFaceUp,
              !card.isMatched,
