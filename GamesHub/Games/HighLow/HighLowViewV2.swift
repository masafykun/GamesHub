...
enum HLV2Suit: String, CaseIterable {
    case spades = "♠", hearts = "♥", diamonds = "♦", clubs = "♣"
    var isRed: Bool { self == .hearts || self == .diamonds }
}

enum HLV2Rank: Int, CaseIterable {
...
struct HLV2Card: Equatable {
    let rank: HLV2Rank
    let suit: HLV2Suit
}

enum HLV2Phase { case start, playing, gameOver }

// MARK: - View

struct HighLowViewV2: View {
    @State private var phase: HLV2Phase = .start
    @State private var deck: [HLV2Card] = []
    @State private var currentCard: HLV2Card? = nil
    @State private var score: Int = 0
    @AppStorage("hlV2HighScore") private var highScore: Int = 0
    ...
    private func glassCardView(_ card: HLV2Card) -> some View {
        ZStack {
    ...
    private func buildDeck() -> [HLV2Card] {
        var cards: [HLV2Card] = []
        for suit in HLV2Suit.allCases {
            for rank in HLV2Rank.allCases {
                cards.append(HLV2Card(rank: rank, suit: suit))
            }
        }
    ...
    private func updateDifficulty(correct: Bool) {
        ...
        if recentResults.count == 5 {
            ...
            if trueCount > 4 {
                ...
                // Difficulty applied: fewer cards remain (shuffle a harder subset)
                // Practically we signal hard mode to UI and reduce deck variety
                let keepCount = max(10, Int(Double(deck.count) / difficultyMultiplier))
                deck = Array(deck.prefix(keepCount)).shuffled()
            }
        }
    }
