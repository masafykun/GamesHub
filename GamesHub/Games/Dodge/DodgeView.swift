...
struct DodgeView: View {
    ...
    @State private var currentSpawnInterval: Double = 1.5

    @State private var difficulty: DodgeDifficulty = .normal
    ...
    private func updateGame() {
        ...
        // Spawn faster as the round progresses (every 10 points)
        let newInterval = max(spawnInterval * 0.4, spawnInterval - Double(score / 10) * 0.1)
        if abs(newInterval - currentSpawnInterval) > 0.05 {
    ...
    private func scheduleSpawnTimer() {
        spawnTimer = Timer.scheduledTimer(withTimeInterval: currentSpawnInterval, repeats: true) { _ in
            guard gameState == .playing else { return }
            spawnProjectile()
        }
    }
    ...
    private func spawnProjectile() {
        ...
        let proj = DodgeProjectile(position: spawnPoint, velocity: velocity, color: color)
