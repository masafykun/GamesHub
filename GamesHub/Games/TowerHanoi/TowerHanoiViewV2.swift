...
// MARK: - TowerHanoiViewV2 (Glassmorphism)

struct TowerHanoiViewV2: View {
    @StateObject private var vm = TowerHanoiV2ViewModel()
    ...
    var body: some View {
        ZStack {
            ...
            if vm.showStartScreen {
                TowerHanoiV2StartScreen(diskCount: $vm.diskCount) {
                    vm.startGame()
                }
            } else {
                TowerHanoiV2GameView(vm: vm)
            }
        }
...
// MARK: - V2 Start Screen

private struct TowerHanoiV2StartScreen: View {
        ...
        VStack(spacing: 28) {
            ...
            ZStack {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    TowerHanoiV2RuleRow(icon: "1.circle.fill", text: "Move all disks to peg C")
                    TowerHanoiV2RuleRow(icon: "2.circle.fill", text: "Move only the top disk")
                    TowerHanoiV2RuleRow(icon: "3.circle.fill", text: "No disk on a smaller one")
                }
                .padding(20)
            }
            .padding(.horizontal)
...
private struct TowerHanoiV2RuleRow: View {
    let icon: String
    let text: String
...
private struct TowerHanoiV2GameView: View {
    @ObservedObject var vm: TowerHanoiV2ViewModel
...
private struct TowerHanoiV2PegView: View {
    let pegIndex: Int
    let disks: [Int]
    let diskCount: Int
    let isSelected: Bool
            ...
            ZStack(alignment: .bottom) {
                ...
                VStack(spacing: 3) {
                    ForEach(Array(disks.reversed().enumerated()), id: \.offset) { _, disk in
                        TowerHanoiV2DiskView(size: disk, maxSize: diskCount)
                    }
                }
                .padding(.bottom, 4)
            }
...
private struct TowerHanoiV2DiskView: View {
    let size: Int
    let maxSize: Int
...
private class TowerHanoiV2ViewModel: ObservableObject {
