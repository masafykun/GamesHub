...
private struct BlackjackV2Card: Identifiable {
    let id = UUID()
    let suit: String
    let rank: String
    let value: Int
    ...
    static func deck() -> [BlackjackV2Card] {
        let suits = ["♠︎", "♥︎", "♦︎", "♣︎"]
        ...
        var cards: [BlackjackV2Card] = []
        for suit in suits {
            for (rank, value) in ranks {
                cards.append(BlackjackV2Card(suit: suit, rank: rank, value: value, isFaceDown: false))
            }
        }
...
private enum BlackjackV2Phase {
    case betting, playing, dealerTurn, result
}

private enum BlackjackV2Result {
    case playerWin, dealerWin, push, blackjack, playerBust, dealerBust
}

// MARK: - ViewModel

private class BlackjackV2ViewModel: ObservableObject {
    @Published var playerHand: [BlackjackV2Card] = []
    @Published var dealerHand: [BlackjackV2Card] = []
    @Published var chips: Int = 1000
    @Published var bet: Int = 100
    @Published var phase: BlackjackV2Phase = .betting
    @Published var result: BlackjackV2Result? = nil

    private var deck: [BlackjackV2Card] = []
    ...
    func calculateTotal(hand: [BlackjackV2Card]) -> Int {
        var total = 0
        var aces = 0
    ...
    private func draw() -> BlackjackV2Card {
        if deck.isEmpty { deck = BlackjackV2Card.deck() }
...
struct BlackjackViewV2: View {
    @StateObject private var vm = BlackjackV2ViewModel()
