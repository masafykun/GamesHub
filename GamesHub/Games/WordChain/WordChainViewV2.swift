...
// MARK: - Types
enum WdChV2Phase { case start, playing, gameOver }

struct WordChainViewV2: View {
    @State private var phase: WdChV2Phase = .start
    ...
    @State private var wordTimeLimit: Double = 10.0
    ...
    private let gradientColors: [Color] = [Color(red: 0.25, green: 0.2, blue: 0.6), Color(red: 0.1, green: 0.5, blue: 0.7)]
    ...
    func glassPanel<C: View>(@ViewBuilder content: () -> C) -> some View {
        content()
            .padding()
    ...
    var gameScreen: some View {
        VStack(spacing: 16) {
            ...
            glassPanel {
                HStack {
                    VStack(alignment: .leading, spacing: 2) {
                        Text("SCORE").font(.caption2).foregroundStyle(.white.opacity(0.6))
                        Text("\(score)").font(.title2.bold()).foregroundStyle(.white)
                    }
                    Spacer()
                    VStack(spacing: 2) {
                        Text("DIFFICULTY").font(.caption2).foregroundStyle(.white.opacity(0.6))
                        Text("Level \(difficultyLevel)").font(.caption.bold()).foregroundStyle(.yellow)
                    }
                    Spacer()
                    VStack(alignment: .trailing, spacing: 2) {
                        Text("CHAIN").font(.caption2).foregroundStyle(.white.opacity(0.6))
                        Text("\(chainLength)").font(.title2.bold()).foregroundStyle(.white)
                    }
                }
            }
            .padding(.horizontal)
            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    RoundedRectangle(cornerRadius: 4)
                        ...
                        .frame(height: 8)
                    RoundedRectangle(cornerRadius: 4)
                        .fill(timeRemaining > wordTimeLimit * 0.4 ? Color.green : Color.red)
                        .frame(width: geo.size.width * CGFloat(timeRemaining / wordTimeLimit), height: 8)
            ...
            glassPanel {
                VStack(spacing: 8) {
                    Text("Current Word").font(.caption).foregroundStyle(.white.opacity(0.6))
                    Text(currentWord.uppercased())
                        .font(.system(size: 44, weight: .bold, design: .rounded))
                        .foregroundStyle(.white)
                    Text("Next must start with '\(String(currentWord.last ?? "?").uppercased())'")
        ...
        VStack(spacing: 24) {
            glassPanel {
                VStack(spacing: 10) {
                    ...
                    HStack(spacing: 32) {
                        VStack {
                            Text("\(score)").font(.title.bold()).foregroundStyle(.yellow)
                            Text("Score").font(.caption).foregroundStyle(.white.opacity(0.7))
                        }
                        VStack {
                            Text("\(chainLength)").font(.title.bold()).foregroundStyle(.cyan)
                            Text("Chain").font(.caption).foregroundStyle(.white.opacity(0.7))
                        }
                        VStack {
                            Text("Lv.\(difficultyLevel)").font(.title.bold()).foregroundStyle(.green)
                            Text("Difficulty").font(.caption).foregroundStyle(.white.opacity(0.7))
                        }
                    }
                }
                .frame(maxWidth: .infinity)
            ...
            if !usedWords.isEmpty {
                glassPanel {
                    VStack(alignment: .leading, spacing: 6) {
                        Text("Your Chain").font(.caption.bold()).foregroundStyle(.white.opacity(0.7))
    ...
    func recordResult(_ success: Bool) {
        ...
        if recentResults.count == 5 && recentResults.filter({ $0 }).count > 4 {
            wordTimeLimit = max(4.0, wordTimeLimit * 0.8)
            difficultyLevel += 1
            recentResults = []
        }
    }
