...
enum NumberFlowGamePhase {
    case start, playing, gameOver
}

struct NumberFlowFallingNumber: Identifiable {
    let id = UUID()
    var value: Int
    var column: Int
    var row: Double
...
struct NumberFlowView: View {
    ...
    @State private var phase: NumberFlowGamePhase = .start
    @State private var score: Int = 0
    @State private var timeLeft: Double = 30
    @State private var baseSpeed: Double = 0.008
    @State private var speed: Double = 0.008
    @State private var falling: NumberFlowFallingNumber? = nil
    @State private var stacks: [[NumberFlowStackEntry]] = Array(repeating: [], count: 9)
    ...
    func spawnFallingNumber() {
        ...
        falling = NumberFlowFallingNumber(value: val, column: col, row: 0.02)
    }
    ...
    func land(number: NumberFlowFallingNumber) {
        // Any number sorted into its own column counts — otherwise the player
        // could only score on the turns where the spawn matched a hidden target.
        let isCorrect = number.column == (number.value - 1)
        stacks[number.column].append(NumberFlowStackEntry(value: number.value, correct: isCorrect))
