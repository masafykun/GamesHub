...
struct PongViewV2: View {
        ...
        GeometryReader { geo in
            ...
            ZStack {
                ...
                if !isPlaying && !gameOver {
                    ...
                    pongV2GameOverScreen
                } else {
                    pongV2GameCanvas(size: size)
                        .gesture(
            ...
            .onAppear {
                pongV2ResetBall(size: size)
                playerY = size.height / 2
                aiY = size.height / 2
            }
            .onReceive(timer) { _ in
                guard isPlaying && !gameOver else { return }
                pongV2UpdateGame(size: size)
            }
        }
    }

    // MARK: - Start Screen
    private var pongV2StartScreen: some View {
        VStack(spacing: 48) {
            VStack(spacing: 8) {
                Text("PONG")
    ...
    private var pongV2GameOverScreen: some View {
        VStack(spacing: 32) {
            Text(winnerText)
            ...
            Button(action: pongV2RestartGame) {
                Text("PLAY AGAIN")
                    .font(.system(size: 18, weight: .bold, design: .monospaced))
                    .foregroundColor(.white)
                    .padding(.horizontal, 40)
    ...
    // MARK: - Game Canvas
    private func pongV2GameCanvas(size: CGSize) -> some View {
        ZStack {
    ...
    private func pongV2RestartGame() {
        playerScore = 0
        aiScore = 0
        gameOver = false
        winnerText = ""
    ...
    private func pongV2ResetBall(size: CGSize) {
    ...
    private func pongV2UpdateGame(size: CGSize) {
        ...
        aiY = (aiY + aiMove).pongV2Clamped(to: paddleHeight / 2...(size.height - paddleHeight / 2))
        playerY = playerY.pongV2Clamped(to: paddleHeight / 2...(size.height - paddleHeight / 2))
        ...
        if ballPos.x - ballRadius <= 0 {
            aiScore += 1
            pongV2CheckWin(size: size)
            if !gameOver { pongV2ResetBall(size: size) }
    ...
    private func pongV2CheckWin(size: CGSize) {
...
private extension Comparable {
    func pongV2Clamped(to range: ClosedRange<Self>) -> Self {
