...
struct RopeCutView: View {
    @State private var phase: RopeCutPhase = .start
    @AppStorage("ropeCutBestScore") private var bestScore: Int = 0
    ...
    @State private var ropes: [RopeCutRope] = []
    @State private var pins: [RopeCutPin] = []
    ...
    var levels: [RopeCutLevel] {
        [
            RopeCutLevel(pins: [RopeCutPin(position: CGPoint(x: 0.5, y: 0.05))],
                       ropeIndices: [0], ballStart: CGPoint(x: 0.5, y: 0.35),
                       starPosition: CGPoint(x: 0.5, y: 0.75)),
            RopeCutLevel(pins: [RopeCutPin(position: CGPoint(x: 0.3, y: 0.05)), RopeCutPin(position: CGPoint(x: 0.7, y: 0.05))],
                       ropeIndices: [0, 1], ballStart: CGPoint(x: 0.5, y: 0.35),
                       starPosition: CGPoint(x: 0.2, y: 0.75)),
            RopeCutLevel(pins: [RopeCutPin(position: CGPoint(x: 0.5, y: 0.05)), RopeCutPin(position: CGPoint(x: 0.85, y: 0.4))],
                       ropeIndices: [0, 1], ballStart: CGPoint(x: 0.6, y: 0.35),
                       starPosition: CGPoint(x: 0.15, y: 0.7)),
            RopeCutLevel(pins: [RopeCutPin(position: CGPoint(x: 0.2, y: 0.05)), RopeCutPin(position: CGPoint(x: 0.8, y: 0.05)), RopeCutPin(position: CGPoint(x: 0.5, y: 0.1))],
                       ropeIndices: [0, 1, 2], ballStart: CGPoint(x: 0.5, y: 0.35),
                       starPosition: CGPoint(x: 0.8, y: 0.72)),
            RopeCutLevel(pins: [RopeCutPin(position: CGPoint(x: 0.15, y: 0.05)), RopeCutPin(position: CGPoint(x: 0.85, y: 0.05)), RopeCutPin(position: CGPoint(x: 0.5, y: 0.08))],
        ...
        ZStack {
            ...
            ForEach(ropes.indices, id: \.self) { i in
                if !ropes[i].cut {
                    ...
                    RopeCutRopeLine(start: pin.position, end: ballPos)
    ...
    func setupLevel(index: Int, geo: GeometryProxy) {
        ...
        pins = level.pins.map { RopeCutPin(position: CGPoint(x: $0.position.x * w, y: $0.position.y * h)) }
        ropes = level.ropeIndices.map { RopeCutRope(pinIndex: $0) }
