...
enum PiCrV2Phase { case start, playing, solved }
...
enum PiCrV2CellState { case empty, filled, marked }

// MARK: - Puzzle Data

private func piCrV2BuildPuzzles() -> [PiCrV2Puzzle] {
    ...
    return [
        PiCrV2Puzzle(name: "Heart", solution: heart, rowClues: hc.rows, colClues: hc.cols),
        PiCrV2Puzzle(name: "Smiley", solution: smiley, rowClues: sc.rows, colClues: sc.cols),
        PiCrV2Puzzle(name: "House", solution: house, rowClues: hoc.rows, colClues: hoc.cols)
    ]
}

private let piCrV2Puzzles: [PiCrV2Puzzle] = piCrV2BuildPuzzles()

// MARK: - Adaptive Difficulty

private struct PiCrV2Difficulty {
...
struct PicCrossViewV2: View {
    @State private var phase: PiCrV2Phase = .start
    @State private var puzzleIndex: Int = 0
    @State private var grid: [[PiCrV2CellState]] = Array(repeating: Array(repeating: .empty, count: 8), count: 8)
    @State private var mistakes: Int = 0
    @State private var recentResults: [Bool] = []
    @State private var difficulty: PiCrV2Difficulty = PiCrV2Difficulty()
    @State private var timeElapsed: Double = 0
    @State private var timer: Timer? = nil
    @State private var hintsRemaining: Int = 3

    private var puzzle: PiCrV2Puzzle { piCrV2Puzzles[puzzleIndex] }
    ...
    var startScreen: some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 12) {
                ForEach(piCrV2Puzzles.indices, id: \.self) { i in
                    Button(piCrV2Puzzles[i].name) {
                        puzzleIndex = i
                        startGame()
                    }
