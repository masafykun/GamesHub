...
// MARK: - Supporting Types (V2)
enum BGmV2Phase {
    case start, playing, finished
}

struct BGmV2Question {
    let binary: String
    let correct: Int
    let choices: [Int]
}
...
struct BinaryGameViewV2: View {
    @State private var phase: BGmV2Phase = .start
    @State private var questions: [BGmV2Question] = []
    ...
    var currentQuestion: BGmV2Question? {
    ...
    var body: some View {
        ZStack {
            ...
            switch phase {
            case .start:
                BGmV2StartScreen(onStart: startGame)
            case .playing:
            ...
            case .finished:
                BGmV2ResultScreen(score: score, total: totalQuestions, onRestart: resetGame)
            }
        }
    }

    func startGame() {
        questions = (0..<totalQuestions).map { _ in BGmV2Generator.generate(fast: speedMultiplier > 1.2) }
        currentIndex = 0
        score = 0
        selectedAnswer = nil
        showFeedback = false
...
// MARK: - Generator V2
enum BGmV2Generator {
    static func generate(fast: Bool) -> BGmV2Question {
        ...
        return BGmV2Question(binary: binary, correct: value, choices: choices.shuffled())
    }
}
...
// MARK: - Start Screen V2
struct BGmV2StartScreen: View {
    let onStart: () -> Void
    var body: some View {
        VStack(spacing: 32) {
            VStack(spacing: 8) {
...
// MARK: - Play Screen V2
struct BGmV2PlayScreen: View {
    let question: BGmV2Question
...
// MARK: - Choice Button V2
struct BGmV2ChoiceButton: View {
    let value: Int
    let correct: Int
    let selected: Int?
    let showFeedback: Bool
...
// MARK: - Result Screen V2
struct BGmV2ResultScreen: View {
    let score: Int
    let total: Int
    let onRestart: () -> Void
