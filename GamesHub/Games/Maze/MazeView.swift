...
enum MazePhase { case start, playing, won }
enum MazeDir { case up, down, left, right }

struct MazeCell: Equatable {
    var x: Int; var y: Int
}

struct MazeMaze {
    let size: Int
    var walls: [[Set<MazeDir>]]

    init(size: Int, seed: Int = 1) {
        self.size = size
        self.walls = Array(repeating: Array(repeating: Set<MazeDir>(), count: size), count: size)
        generate(seed: seed)
    }

    mutating func generate(seed: Int) {
        var allWalls = Array(repeating: Array(repeating: Set([MazeDir.up, .down, .left, .right]), count: size), count: size)
        ...
        func carve(x: Int, y: Int) {
            visited[y][x] = true
            var dirs: [MazeDir] = [.up, .down, .left, .right]
            ...
            for d in dirs {
                ...
                let opp: MazeDir = d == .up ? .down : d == .down ? .up : d == .left ? .right : .left
    ...
    func canMove(from cell: MazeCell, dir: MazeDir) -> Bool {
        !walls[cell.y][cell.x].contains(dir)
    }
}

struct MazeView: View {
    @State private var phase: MazePhase = .start
    @State private var player = MazeCell(x: 0, y: 0)
    @State private var maze = MazeMaze(size: 13, seed: Int.random(in: 1...1_000_000))
    @State private var elapsed: Double = 0
    @State private var level: Int = 1
    @State private var gameTimer: Timer? = nil
    @State private var recentResults: [Bool] = []
    @State private var speedMultiplier: Double = 1.0
    @AppStorage("mazeBestTime") private var bestTime: Double = 0

    /// Each level widens the maze instead of speeding up the clock.
    var gridSize: Int { min(19, 11 + level * 2) }
    var exitCell: MazeCell { MazeCell(x: gridSize - 1, y: gridSize - 1) }
    ...
    var wonView: some View {
        VStack(spacing: 22) {
            ...
            Text("Level \(level) · \(gridSize)×\(gridSize)").foregroundColor(.white.opacity(0.7))
            if bestTime > 0 {
                Text(String(format: "Best: %.1fs", bestTime)).font(.caption).foregroundColor(.white.opacity(0.6))
            }
            HStack(spacing: 16) {
                Button("Next Level") { level += 1; startGame() }
                    .buttonStyle(MazeGlassButtonStyle())
    ...
    func startGame() {
        player = MazeCell(x: 0, y: 0)
        maze = MazeMaze(size: gridSize, seed: Int.random(in: 1...1_000_000))
        elapsed = 0
        phase = .playing
    ...
    func finishGame(won: Bool) {
        gameTimer?.invalidate()
        if won, bestTime == 0 || elapsed < bestTime { bestTime = elapsed }
...
struct MazeGlassButtonStyle: ButtonStyle {
