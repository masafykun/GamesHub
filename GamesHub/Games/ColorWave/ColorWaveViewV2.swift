...
struct ClWvV2Band: Identifiable {
    let id = UUID()
    var hue: Double
    var yOffset: CGFloat
}
...
struct ColorWaveViewV2: View {
    @State private var phase: ClWvV2Phase = .start
    @State private var score: Int = 0
    @State private var lives: Int = 5
    @State private var bands: [ClWvV2Band] = []
    ...
    func startGame() {
        ...
        bands = (0..<10).map { i in
            ClWvV2Band(hue: Double(i) / 10.0, yOffset: CGFloat(i) * bandHeight)
        }
