...
enum CrcV2Phase { case start, playing, complete }

struct CrcV2GridPos: Hashable {
    let row: Int, col: Int
}

enum CrcV2ComponentType { case batteryPlus, batteryMinus, led, resistor }

struct CrcV2Component {
    let pos: CrcV2GridPos
    let type: CrcV2ComponentType
}

struct CrcV2Layout {
    let components: [CrcV2Component]
    let requiredPath: [CrcV2GridPos]
}

struct CrcV2WireSegment: Hashable {
    let from: CrcV2GridPos
    let to: CrcV2GridPos
    init(from: CrcV2GridPos, to: CrcV2GridPos) {
...
struct CircuitViewV2: View {
    @State private var phase: CrcV2Phase = .start
    @State private var wires: Set<CrcV2WireSegment> = []
    @State private var layoutIndex: Int = 0
    @State private var isComplete: Bool = false
    @State private var score: Int = 0
    @State private var dragStart: CrcV2GridPos? = nil
    ...
    let layouts: [CrcV2Layout] = [
        CrcV2Layout(
            components: [
                CrcV2Component(pos: CrcV2GridPos(row: 1, col: 1), type: .batteryPlus),
                CrcV2Component(pos: CrcV2GridPos(row: 1, col: 5), type: .resistor),
                CrcV2Component(pos: CrcV2GridPos(row: 5, col: 5), type: .led),
                CrcV2Component(pos: CrcV2GridPos(row: 5, col: 1), type: .batteryMinus)
            ],
            requiredPath: [
                CrcV2GridPos(row: 1, col: 1), CrcV2GridPos(row: 1, col: 2), CrcV2GridPos(row: 1, col: 3),
                CrcV2GridPos(row: 1, col: 4), CrcV2GridPos(row: 1, col: 5), CrcV2GridPos(row: 2, col: 5),
                CrcV2GridPos(row: 3, col: 5), CrcV2GridPos(row: 4, col: 5), CrcV2GridPos(row: 5, col: 5),
                CrcV2GridPos(row: 5, col: 4), CrcV2GridPos(row: 5, col: 3), CrcV2GridPos(row: 5, col: 2),
                CrcV2GridPos(row: 5, col: 1)
            ]
        ),
        CrcV2Layout(
            components: [
                CrcV2Component(pos: CrcV2GridPos(row: 0, col: 3), type: .batteryPlus),
                CrcV2Component(pos: CrcV2GridPos(row: 3, col: 0), type: .resistor),
                CrcV2Component(pos: CrcV2GridPos(row: 6, col: 3), type: .led),
                CrcV2Component(pos: CrcV2GridPos(row: 3, col: 6), type: .batteryMinus)
            ],
            requiredPath: [
                CrcV2GridPos(row: 0, col: 3), CrcV2GridPos(row: 1, col: 3), CrcV2GridPos(row: 2, col: 3),
                CrcV2GridPos(row: 3, col: 3), CrcV2GridPos(row: 3, col: 2), CrcV2GridPos(row: 3, col: 1),
                CrcV2GridPos(row: 3, col: 0), CrcV2GridPos(row: 4, col: 0), CrcV2GridPos(row: 5, col: 0),
                CrcV2GridPos(row: 6, col: 0), CrcV2GridPos(row: 6, col: 1), CrcV2GridPos(row: 6, col: 2),
                CrcV2GridPos(row: 6, col: 3), CrcV2GridPos(row: 6, col: 4), CrcV2GridPos(row: 6, col: 5),
                CrcV2GridPos(row: 6, col: 6), CrcV2GridPos(row: 5, col: 6), CrcV2GridPos(row: 4, col: 6),
                CrcV2GridPos(row: 3, col: 6)
            ]
        ),
        CrcV2Layout(
            components: [
                CrcV2Component(pos: CrcV2GridPos(row: 2, col: 0), type: .batteryPlus),
                CrcV2Component(pos: CrcV2GridPos(row: 0, col: 4), type: .resistor),
                CrcV2Component(pos: CrcV2GridPos(row: 4, col: 6), type: .led),
                CrcV2Component(pos: CrcV2GridPos(row: 6, col: 2), type: .batteryMinus)
            ],
            requiredPath: [
                CrcV2GridPos(row: 2, col: 0), CrcV2GridPos(row: 1, col: 0), CrcV2GridPos(row: 0, col: 0),
                CrcV2GridPos(row: 0, col: 1), CrcV2GridPos(row: 0, col: 2), CrcV2GridPos(row: 0, col: 3),
                CrcV2GridPos(row: 0, col: 4), CrcV2GridPos(row: 0, col: 5), CrcV2GridPos(row: 0, col: 6),
                CrcV2GridPos(row: 1, col: 6), CrcV2GridPos(row: 2, col: 6), CrcV2GridPos(row: 3, col: 6),
                CrcV2GridPos(row: 4, col: 6), CrcV2GridPos(row: 5, col: 6), CrcV2GridPos(row: 6, col: 6),
                CrcV2GridPos(row: 6, col: 5), CrcV2GridPos(row: 6, col: 4), CrcV2GridPos(row: 6, col: 3),
                CrcV2GridPos(row: 6, col: 2)
            ]
        )
    ]

    var currentLayout: CrcV2Layout { layouts[layoutIndex % layouts.count] }
    ...
    func wireView(_ seg: CrcV2WireSegment, cs: CGFloat) -> some View {
    ...
    func componentView(_ comp: CrcV2Component, cs: CGFloat) -> some View {
    ...
    func checkCompletion() {
        ...
        for i in 0..<path.count - 1 {
            if !wires.contains(CrcV2WireSegment(from: path[i], to: path[i + 1])) { return }
        }
        isComplete = true
        ledPulse = true
