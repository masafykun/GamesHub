...
// MARK: - Private Models (V2 scoped)

private enum FarkleV2Phase {
    case rolling, selecting, banked
}

private struct FarkleV2Die: Identifiable {
    let id: Int
    var value: Int
    var isKept: Bool
    var isScoring: Bool
...
private class FarkleV2ViewModel: ObservableObject {
    @Published var dice: [FarkleV2Die] = []
    @Published var roundScore: Int = 0
    @Published var totalScore: Int = 0
    @Published var phase: FarkleV2Phase = .rolling
    ...
    func resetDice() {
        dice = (0..<6).map { FarkleV2Die(id: $0, value: Int.random(in: 1...6), isKept: false, isScoring: false) }
    }
    ...
    @discardableResult
    private func markScoringDice(unKeptDice: [FarkleV2Die], scoringIndices: Set<Int>) -> Bool {
        var unKeptIndex = 0
        for i in dice.indices {
            if !dice[i].isKept {
    ...
    func toggleKeep(die: FarkleV2Die) {
...
private struct FarkleV2DieView: View {
    let die: FarkleV2Die
    let onTap: () -> Void
...
private struct FarkleV2GlassCard<Content: View>: View {
...
struct FarkleViewV2: View {
    @StateObject private var vm = FarkleV2ViewModel()
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 16) {
                    // Header card
                    FarkleV2GlassCard(accentColor: .purple) {
                        HStack {
                    ...
                    // Progress bar
                    FarkleV2GlassCard(accentColor: .cyan) {
                    ...
                    // Message
                    FarkleV2GlassCard(accentColor: vm.farkleFlash ? .red : .indigo) {
                        HStack {
                    ...
                    // Dice grid
                    FarkleV2GlassCard(accentColor: .blue) {
                        VStack(spacing: 12) {
                            ...
                            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 10), count: 3), spacing: 10) {
                                ForEach(vm.dice) { die in
                                    FarkleV2DieView(die: die) { vm.toggleKeep(die: die) }
    ...
    private func scoreCard(label: String, value: Int, color: Color) -> some View {
        FarkleV2GlassCard(accentColor: color) {
            VStack(spacing: 4) {
