...
enum DtV2Phase { case start, playing, gameOver }

struct DtV2Dart: Identifiable {
    let id = UUID()
    let position: CGPoint
    let score: Int
}

struct DartsViewV2: View {
    @State private var phase: DtV2Phase = .start
    @State private var crosshairPos: CGPoint = CGPoint(x: 150, y: 150)
    @State private var dartsThrown: [DtV2Dart] = []
    ...
    func throwDart() {
        ...
        let dart = DtV2Dart(position: crosshairPos, score: score)
