...
enum WaterFlowPipeType: CaseIterable {
...
struct WaterFlowCell {
    var pipeType: WaterFlowPipeType
    var rotation: Int
...
enum WaterFlowPhase { case start, playing, solved }

// MARK: -  View (Glassmorphism + Adaptive Difficulty)

struct WaterFlowView: View {
    static let gridSize = 6
    @State private var phase: WaterFlowPhase = .start
    @State private var grid: [[WaterFlowCell]] = WaterFlowView.buildLevel(0)
    ...
    static func buildLevel(_ index: Int, scramble: Bool = false) -> [[WaterFlowCell]] {
        let gs = gridSize
        let pipeTypes: [WaterFlowPipeType] = [.straight, .elbow, .tJunction, .cross]
        let rotations = [0,1,2,3]
        var cells = [[WaterFlowCell]]()
        let seed = index * 7 + 3
        var rng = seed
        func nextRand(_ n: Int) -> Int { rng = rng &* 1664525 &+ 1013904223; return abs(rng) % n }

        for r in 0..<gs {
            var row = [WaterFlowCell]()
            for c in 0..<gs {
                let pt = pipeTypes[nextRand(pipeTypes.count)]
                let rot = scramble ? rotations[nextRand(4)] : 0
                var cell = WaterFlowCell(pipeType: pt, rotation: rot)
    ...
    func nextLevel() {
        ...
        currentLevel += 1
        solvedCount += 1
        grid = WaterFlowView.buildLevel(currentLevel, scramble: difficultyMultiplier > 1.0)
        phase = .playing
    }
