...
enum TrafficPhase { case start, playing, gameOver }
enum TrafficDir { case north, south, east, west }

struct TrafficCar: Identifiable {
    let id = UUID()
    let direction: TrafficDir
    var position: CGFloat = 0.0
    var stopped: Bool = false
}

struct TrafficView: View {
    @State private var phase: TrafficPhase = .start
    @State private var cars: [TrafficCar] = []
    ...
    @AppStorage("trafficBestScore") private var bestScore: Int = 0
    ...
    func carPos(_ car: TrafficCar) -> CGPoint {
        let mid = boardSize / 2
        let t = car.position
    ...
    func carChip(car: TrafficCar) -> some View {
        let pos = carPos(car)
        return ZStack {
    ...
    func spawnCar() {
        let dir = [TrafficDir.north, .south, .east, .west].randomElement()!
        cars.append(TrafficCar(direction: dir))
        spawnCount += 1
    }
    ...
    func inIntersection(_ car: TrafficCar) -> Bool {
        let pos = carPos(car)
        let mid = boardSize / 2
