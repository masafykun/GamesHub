...
enum CrHpV2Phase { case start, playing, over }
enum CrHpV2RowKind { case safe, road, river }

struct CrHpV2Vehicle {
    var col: Double
    let direction: Int
    var speed: Double
}
...
struct CrHpV2Row {
    let kind: CrHpV2RowKind
    var vehicles: [CrHpV2Vehicle]
    var logs: [CrHpV2Log]
}

// MARK: - CrossyHopViewV2

struct CrossyHopViewV2: View {
    static let cols = 5
    static let rows = 9

    @State private var phase: CrHpV2Phase = .start
    @State private var playerRow = 0
    @State private var playerCol = 2
    @State private var score = 0
    @State private var grid: [CrHpV2Row] = []
    ...
    func buildGrid() -> [CrHpV2Row] {
        var result: [CrHpV2Row] = []
        for i in 0..<Self.rows {
            if i == 0 || i == Self.rows - 1 {
                result.append(CrHpV2Row(kind: .safe, vehicles: [], logs: []))
            } else if i % 2 == 1 {
                let dir = i % 4 == 1 ? 1 : -1
                let base = 1.5 * speedMultiplier
                let cars = (0..<2).map { j in CrHpV2Vehicle(col: Double(j * 3), direction: dir, speed: base + Double(i) * 0.08) }
                result.append(CrHpV2Row(kind: .road, vehicles: cars, logs: []))
            } else {
                let dir = i % 4 == 0 ? 1 : -1
                let base = 1.2 * speedMultiplier
                let logs = [CrHpV2Log(col: 0, width: 2, direction: dir, speed: base), CrHpV2Log(col: 3, width: 2, direction: dir, speed: base)]
                result.append(CrHpV2Row(kind: .river, vehicles: [], logs: logs))
            }
        }
        return result
    }
