...
enum GPV2Phase { case start, playing, result }
...
struct GolfPuttViewV2: View {
    @State private var phase: GPV2Phase = .start
    @State private var currentHole: Int = 0
    @State private var totalStrokes: Int = 0
    @State private var strokesThisHole: Int = 0
    @State private var holes: [GPV2Hole] = GPV2HoleGen.holes()
    ...
    func tickPhysics() {
        ...
        let hc = scaledPt(holes[currentHole].holePos, bounds.size)
        ...
        if ballPos.y < pad + 52 { ballPos.y = pad + 52; ballVelocity.dy = abs(ballVelocity.dy) }
...
// MARK: - Hole Generator V2

struct GPV2HoleGen {
    static func holes() -> [GPV2Hole] {
        [
            GPV2Hole(ballPos: CGPoint(x: 0.5, y: 0.8), holePos: CGPoint(x: 0.5, y: 0.18)),
            GPV2Hole(ballPos: CGPoint(x: 0.2, y: 0.78), holePos: CGPoint(x: 0.78, y: 0.22)),
            GPV2Hole(ballPos: CGPoint(x: 0.78, y: 0.78), holePos: CGPoint(x: 0.22, y: 0.22))
        ]
    }
}

// MARK: - Button Style V2

struct GPV2ButtonStyle: ButtonStyle {
