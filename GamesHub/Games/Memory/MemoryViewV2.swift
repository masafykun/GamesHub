...
struct MemoryV2Card: Identifiable {
    let id: Int
    let symbol: String
...
enum MemoryV2GameState {
    case idle, playing, blocked, won
}

// MARK: - ViewModel

class MemoryV2ViewModel: ObservableObject {
    ...
    @Published var cards: [MemoryV2Card] = []
    @Published var gameState: MemoryV2GameState = .idle
    @Published var matches: Int = 0
    @Published var moves: Int = 0
    @Published var elapsedSeconds: Int = 0
    @Published var roundTimes: [Int] = []
    @Published var difficulty: MemoryV2Difficulty = .medium
    ...
    private func computeDifficulty() -> MemoryV2Difficulty {
    ...
    func setupCards() {
        ...
        var deck: [MemoryV2Card] = []
        for (index, symbol) in symbols.enumerated() {
            deck.append(MemoryV2Card(id: index * 2,     symbol: symbol))
            deck.append(MemoryV2Card(id: index * 2 + 1, symbol: symbol))
        }
        deck.shuffle()
        cards = deck
...
private let memoryV2SymbolColors: [String: Color] = [
...
struct MemoryV2CardView: View {
    let card: MemoryV2Card

    private var symbolColor: Color {
        memoryV2SymbolColors[card.symbol] ?? .purple
    }
...
struct MemoryV2HUD: View {
    let matches: Int
    let totalPairs: Int
    let moves: Int
    let elapsedSeconds: Int
    let difficulty: MemoryV2Difficulty
...
struct MemoryV2WinOverlay: View {
    let matches: Int
    let totalPairs: Int
    let moves: Int
    let elapsedSeconds: Int
...
struct MemoryViewV2: View {
    @StateObject private var viewModel = MemoryV2ViewModel()
