...
struct StackerView: View {
    ...
    private var difficulty: StackerDifficulty {
    ...
    private func startGame() {
        ...
        scrollOffset = 0
        slidingDirection = Bool.random() ? 1.0 : -1.0
        placedBlockScale = [:]
    ...
    private func handleTap() {
        ...
        // Next sliding block: random start side so the rhythm can't be memorized
        let nextColorIndex = (newBlock.colorIndex + 1) % blockColors.count
        let nextDir: CGFloat = Bool.random() ? 1.0 : -1.0
        slidingDirection = nextDir
        slidingBlock = StackerBlock(
            x: nextDir > 0 ? 0 : gameWidth - newBlock.width,
