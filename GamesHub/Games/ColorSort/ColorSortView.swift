...
// MARK: - Models ()
enum ColorSortColor: CaseIterable {
    ...
    var color: Color {
        ...
        case .purple: return Color(red: 0.75, green: 0.4,  blue: 0.95)
        }
    }
}

struct ColorSortTube {
    var balls: [ColorSortColor] = []
    let capacity = 4
    var isFull: Bool  { balls.count == capacity }
    var isEmpty: Bool { balls.isEmpty }
    var topBall: ColorSortColor? { balls.last }
    var isSorted: Bool {
        balls.count == capacity && Set(balls.map { "\($0)" }).count == 1
    }
}

enum ColorSortPhase { case start, playing, won }

// MARK: -  View
struct ColorSortView: View {
    @State private var tubes: [ColorSortTube] = []
    @State private var selectedTube: Int? = nil
    @State private var phase: ColorSortPhase = .start
        ...
        VStack(spacing: 20) {
            ...
            HStack(spacing: 6) {
                ForEach(0..<tubes.count, id: \.self) { i in
                    ColorSortTubeView(tube: tubes[i], isSelected: selectedTube == i)
    ...
    // MARK: Logic
    /// Colours in play grow with difficulty; two spare tubes keep every
    /// shuffle solvable.
    private var colorCount: Int { difficulty >= 1.4 ? 5 : 4 }

    private func startGame() {
        let palette = Array(ColorSortColor.allCases.prefix(colorCount))
        var allBalls: [ColorSortColor] = palette.flatMap { Array(repeating: $0, count: 4) }
        allBalls.shuffle()

        let filledCount = palette.count
        let emptyCount = 2
        tubes = (0..<(filledCount + emptyCount)).map { i in
            var t = ColorSortTube()
...
// MARK: - Tube View 
private struct ColorSortTubeView: View {
    let tube: ColorSortTube
    let isSelected: Bool
