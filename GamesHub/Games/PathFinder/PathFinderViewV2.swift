...
struct PthFV2Level {
    ...
    static func level(at index: Int, difficulty: Double) -> PthFV2Level {
        let base = PthFV2Level.baseLevels[index % PthFV2Level.baseLevels.count]
        if difficulty > 1.2 {
            ...
            return PthFV2Level(walls: extraWalls, startCell: base.startCell, endCell: base.endCell)
        }
        return base
    }

    static let baseLevels: [PthFV2Level] = [
        PthFV2Level(walls: [2, 3, 10, 18, 19, 27, 35, 43, 44, 52, 53, 60, 61], startCell: 0, endCell: 63),
        PthFV2Level(walls: [1, 9, 17, 25, 14, 22, 30, 38, 46, 54, 37, 45, 53], startCell: 8, endCell: 55),
        PthFV2Level(walls: [4, 5, 6, 12, 20, 28, 29, 30, 38, 46, 42, 43, 51], startCell: 0, endCell: 56)
    ]
}

// MARK: - V2 View

struct PathFinderViewV2: View {
    @State private var phase: PthFV2GamePhase = .start
    ...
    var level: PthFV2Level { PthFV2Level.level(at: levelIndex, difficulty: difficulty) }
    var pathLength: Int { max(0, path.count - 1) }
    var currentScore: Int { max(0, 100 - pathLength) }

    func cellType(index: Int) -> PthFV2CellType {
    ...
    func nextLevel() {
        levelIndex = (levelIndex + 1) % PthFV2Level.baseLevels.count
        path = []
        phase = .start
    }
