...
enum BkPsV2Phase { case start, playing, won }

enum BkPsV2Color: String, CaseIterable {
    case red, blue, green
    var color: Color {
        switch self {
...
struct BkPsV2Block: Identifiable {
    let id: UUID
    var col: Int
    var row: Int
    var color: BkPsV2Color
}
...
struct BkPsV2Engine {
    ...
    static func makeLevel(_ index: Int) -> [BkPsV2Block] {
        let configs: [[(Int, Int, BkPsV2Color)]] = [
        ...
        return configs[index % configs.count].map { BkPsV2Block(id: UUID(), col: $0.0, row: $0.1, color: $0.2) }
    }

    static func slide(_ blocks: [BkPsV2Block], dir: BkPsV2Dir) -> [BkPsV2Block] {
        let sorted: [BkPsV2Block]
        switch dir {
    ...
    static func isWon(_ blocks: [BkPsV2Block]) -> Bool {
        for c in BkPsV2Color.allCases {
...
enum BkPsV2Dir {
    ...
    func pos(_ b: BkPsV2Block) -> Int { self == .left || self == .right ? b.col : b.row }
    func perp(_ b: BkPsV2Block) -> Int { self == .left || self == .right ? b.row : b.col }
    func step(_ p: Int) -> Int {
        switch self { case .left, .up: return p - 1; case .right, .down: return p + 1 }
    }
    func setPos(_ b: inout BkPsV2Block, _ v: Int) {
        if self == .left || self == .right { b.col = v } else { b.row = v }
    }
}

// MARK: - View (V2 - Glassmorphism + Adaptive Difficulty)

struct BlockPushViewV2: View {
    @State private var phase: BkPsV2Phase = .start
    @State private var levelIndex = 0
    @State private var blocks: [BkPsV2Block] = []
    ...
    var gameScreen: some View {
        VStack(spacing: 14) {
            ...
            .gesture(DragGesture(minimumDistance: 10).onEnded { val in
                ...
                let dir: BkPsV2Dir = abs(dx) > abs(dy) ? (dx > 0 ? .right : .left) : (dy > 0 ? .down : .up)
                applySwipe(dir)
            })
    ...
    func startGame() {
        blocks = BkPsV2Engine.makeLevel(levelIndex).map {
    ...
    func applySwipe(_ dir: BkPsV2Dir) {
        let newBlocks = BkPsV2Engine.slide(blocks, dir: dir)
        ...
        if changed { moves += 1 }
        blocks = newBlocks
        if BkPsV2Engine.isWon(blocks) { phase = .won }
    }
}

#Preview { BlockPushViewV2() }
