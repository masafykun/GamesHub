...
enum LtChV2Phase { case start, playing, gameOver }

struct LtChV2Cell: Identifiable {
    let id: Int
    let letter: Character
    var isSelected: Bool = false
}

// MARK: - LetterChainViewV2

struct LetterChainViewV2: View {
    @State private var phase: LtChV2Phase = .start
    @State private var grid: [LtChV2Cell] = []
            ...
            HStack {
                ...
                VStack(alignment: .trailing, spacing: 2) {
                    Text("DIFFICULTY")
                        .font(.caption.bold())
                        .foregroundStyle(.white.opacity(0.7))
                    Text(String(format: "%.0f%%", difficulty * 100))
                        .font(.title3.bold())
                        .foregroundStyle(.white)
                }
            }
    ...
    @ViewBuilder
    func cellView(_ cell: LtChV2Cell) -> some View {
    ...
    func startGame() {
        ...
        grid = (0..<25).map { i in
            LtChV2Cell(id: i, letter: letters.randomElement() ?? "A")
        }
