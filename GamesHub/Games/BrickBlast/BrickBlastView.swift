...
// MARK: - Models
struct BrickBlastBrick: Identifiable {
    let id = UUID()
    var col: Int
    var row: Int
    var hits: Int
...
struct BrickBlastView: View {
    ...
    @State private var phase: BrickBlastPhase = .start
    @State private var bricks: [BrickBlastBrick] = []
    @State private var ball: BrickBlastBall = BrickBlastBall(x: 0, y: 0, dx: 0, dy: 0)
    ...
    func brickView(brick: BrickBlastBrick) -> some View {
    ...
    func generateRow(row: Int) -> [BrickBlastBrick] {
        (0..<cols).map { col in BrickBlastBrick(col: col, row: row, hits: Int.random(in: 1...3)) }
    }
