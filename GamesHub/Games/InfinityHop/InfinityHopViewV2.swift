...
enum IfHpV2Phase { case start, playing, over }

struct IfHpV2Platform: Identifiable {
    let id = UUID()
    var x: CGFloat
    var y: CGFloat
    var width: CGFloat
...
struct InfinityHopViewV2: View {
    @State private var phase: IfHpV2Phase = .start
    @State private var playerX: CGFloat = 0.5
    @State private var playerY: CGFloat = 0.6
    @State private var platforms: [IfHpV2Platform] = []
    ...
    private func startGame() {
        ...
        platforms = []
        for i in 0..<8 {
            platforms.append(IfHpV2Platform(x: CGFloat.random(in: 0.2...0.8), y: CGFloat(i) * 0.14 + 0.2, width: 0.35))
        }
        phase = .playing
        timer?.invalidate()
    ...
    private func tick() {
        ...
        while platforms.count < 8 {
            ...
            platforms.append(IfHpV2Platform(x: CGFloat.random(in: 0.15...0.85), y: -0.05, width: w))
        }
