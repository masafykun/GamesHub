...
struct WordSearchPuzzleData {
    ...
    static func buildGrid(words: [String], size: Int) -> [[Character]] {
        var grid = Array(repeating: Array(repeating: Character("A"), count: size), count: size)
        var placed: [(String, Int, Int, Bool)] = [] // word, row, col, horizontal
        ...
        for word in words {
            ...
            while !success && attempts < 200 {
                ...
                let maxRow = horizontal ? size : size - word.count
                let maxCol = horizontal ? size - word.count : size
                ...
                for (i, ch) in word.enumerated() {
                    ...
                    if existing != Character("A") && existing != ch {
                        canPlace = false
                        break
                    }
                }
                ...
                if canPlace {
                    ...
                    placed.append((word, row, col, horizontal))
                    success = true
                }
            }
        }
        ...
        for r in 0..<size {
            for c in 0..<size {
                if grid[r][c] == Character("A") && !words.contains(where: { _ in false }) {
                    // Check if the cell was placed by any word
                    var isPartOfWord = false
                    for (word, startRow, startCol, horizontal) in placed {
                        for (i, _) in word.enumerated() {
                            let wr = horizontal ? startRow : startRow + i
                            let wc = horizontal ? startCol + i : startCol
                            if wr == r && wc == c { isPartOfWord = true }
                        }
                    }
                    if !isPartOfWord {
...
struct WordSearchViewV2: View {
    ...
    func startGame() {
        ...
        grid = WordSearchPuzzleData.buildGrid(words: words, size: gridSize)
