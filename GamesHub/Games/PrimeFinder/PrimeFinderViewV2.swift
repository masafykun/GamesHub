...
struct PrmFV2Cell: Identifiable {
    let id = UUID()
    let number: Int
    var state: PrmFV2CellState = .neutral
}

enum PrmFV2CellState {
    case neutral, correct, wrong
}

// MARK: - Helpers

private func isPrimeV2(_ n: Int) -> Bool {
...
struct PrimeFinderViewV2: View {
    @State private var phase: PrmFV2Phase = .start
    @State private var cells: [PrmFV2Cell] = []
    ...
    private var primesRemaining: Int {
        cells.filter { isPrimeV2($0.number) && $0.state != .correct }.count
    }
    ...
    @ViewBuilder
    func glassCell(_ cell: PrmFV2Cell) -> some View {
    ...
    func startGame() {
        ...
        cells = chosen.map { PrmFV2Cell(number: $0) }
        score = 0
    ...
    func endGame() {
        gameTimer?.invalidate()
        gameTimer = nil
        let totalPrimes = cells.filter { isPrimeV2($0.number) }.count
