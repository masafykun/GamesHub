...
struct LMrV2Puzzle {
    let starRow: Int
    let starCol: Int
    let preMirrors: [(row: Int, col: Int, type: LMrV2CellType)]
}

struct LMrV2Pos: Hashable { let row: Int; let col: Int }

// MARK: - Laser trace

func computeV2LaserPath(cells: [[LMrV2CellType]], puzzle: LMrV2Puzzle) -> Set<LMrV2Pos> {
    var visited = Set<LMrV2Pos>()
    var row = 3; var col = 0
    var dr = 0; var dc = 1
    var steps = 0
    while col >= 0 && col < 7 && row >= 0 && row < 7 && steps < 100 {
        let pos = LMrV2Pos(row: row, col: col)
...
struct LaserMirrorViewV2: View {
    @State private var phase: LMrV2Phase = .start
    @State private var cells = Array(repeating: Array(repeating: LMrV2CellType.empty, count: 7), count: 7)
    ...
    enum LMrV2Phase { case start, game, results }

    let puzzles: [LMrV2Puzzle] = [
        LMrV2Puzzle(starRow: 2, starCol: 5, preMirrors: [(1,1,.mirrorForward),(3,3,.mirrorBackward)]),
        LMrV2Puzzle(starRow: 5, starCol: 4, preMirrors: [(2,2,.mirrorForward),(4,1,.mirrorBackward),(1,4,.mirrorForward)]),
        LMrV2Puzzle(starRow: 0, starCol: 6, preMirrors: [(3,0,.mirrorForward),(0,3,.mirrorBackward)]),
        LMrV2Puzzle(starRow: 6, starCol: 2, preMirrors: [(1,1,.mirrorForward),(5,5,.mirrorBackward),(3,3,.mirrorForward),(4,2,.mirrorBackward)])
    ]
    ...
    var gameScreen: some View {
        VStack(spacing: 14) {
            HStack {
                ...
                Spacer()
                glassTag(label: "Diff: \(String(format: "%.1f", difficultyMultiplier))x")
            }.padding(.horizontal)
    ...
    var resultsScreen: some View {
        VStack(spacing: 24) {
            ...
            Text("Final difficulty: \(String(format: "%.1f", difficultyMultiplier))x").font(.headline).foregroundColor(.white.opacity(0.7))
    ...
    var gridView: some View {
        ...
        let laserPath = computeV2LaserPath(cells: cells, puzzle: puzzle)
    ...
    func cellView(row: Int, col: Int, puzzle: LMrV2Puzzle, laserPath: Set<LMrV2Pos>) -> some View {
        let isLocked = puzzle.preMirrors.contains { $0.row == row && $0.col == col }
        let isStar = puzzle.starRow == row && puzzle.starCol == col
        let isLit = laserPath.contains(LMrV2Pos(row: row, col: col))
        let cell = cells[row][col]
    ...
    func tapCell(row: Int, col: Int, isLocked: Bool, isStar: Bool) {
        ...
        moveCount += 1
        let path = computeV2LaserPath(cells: cells, puzzle: puzzles[puzzleIndex])
        let puzzle = puzzles[puzzleIndex]
        withAnimation { solved = path.contains(LMrV2Pos(row: puzzle.starRow, col: puzzle.starCol)) }
    }
