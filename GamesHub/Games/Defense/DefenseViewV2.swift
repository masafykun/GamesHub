...
struct DefenseV2Tower: Identifiable {
    let id = UUID()
    var col: Int
    var row: Int
...
struct DefenseV2Enemy: Identifiable {
    let id = UUID()
...
enum DefenseV2GamePhase {
    case playing, gameOver
}

enum DefenseV2DifficultyLevel: String {
    case easy = "EASY"
    case normal = "NORMAL"
    case hard = "HARD"
}
...
struct DefenseViewV2: View {
    let columns = 8
    let rows = 12
    let pathRow = 5

    @State private var towers: [DefenseV2Tower] = []
    @State private var enemies: [DefenseV2Enemy] = []
    @State private var gold: Int = 100
    @State private var lives: Int = 10
    @State private var wave: Int = 1
    @State private var score: Int = 0
    @State private var phase: DefenseV2GamePhase = .playing
    ...
    var difficultyLevel: DefenseV2DifficultyLevel {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            ZStack {
                ...
                VStack(spacing: 8) {
                    // Glassmorphism HUD
                    defenseV2HUD(gridWidth: gridWidth)

                    // Grid
                    ZStack(alignment: .topLeading) {
                        ForEach(0..<rows, id: \.self) { row in
                            ForEach(0..<columns, id: \.self) { col in
                                defenseV2CellView(col: col, row: row, cellSize: cellSize)
                                    .position(
                        ...
                        ForEach(towers) { tower in
                            defenseV2TowerView(cellSize: cellSize)
                                .position(
                        ...
                        ForEach(enemies) { enemy in
                            defenseV2EnemyView(enemy: enemy, cellSize: cellSize)
                                .position(
    ...
    func defenseV2HUD(gridWidth: CGFloat) -> some View {
        HStack(spacing: 0) {
            defenseV2StatPill(label: "💰", value: "\(gold)")
            Spacer()
            defenseV2StatPill(label: "❤️", value: "\(lives)")
            Spacer()
            defenseV2StatPill(label: "🌊", value: "W\(wave)")
            Spacer()
            defenseV2StatPill(label: "⭐", value: "\(score)")
            Spacer()
            // Difficulty badge
    ...
    func defenseV2StatPill(label: String, value: String) -> some View {
        HStack(spacing: 4) {
    ...
    func defenseV2CellView(col: Int, row: Int, cellSize: CGFloat) -> some View {
    ...
    func defenseV2TowerView(cellSize: CGFloat) -> some View {
        ZStack {
            // Glow effect
            Circle()
    ...
    func defenseV2EnemyView(enemy: DefenseV2Enemy, cellSize: CGFloat) -> some View {
    ...
    func defenseV2GameOverOverlay() -> some View {
        ZStack {
    ...
    func placeTower(col: Int, row: Int) {
        ...
        towers.append(DefenseV2Tower(col: col, row: row))
        gold -= 10
    }
