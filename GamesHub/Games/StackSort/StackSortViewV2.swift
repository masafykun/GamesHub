...
// MARK: - Models
enum StStV2DiscColor: CaseIterable {
    case red, blue, green
    var color: Color {
        switch self {
...
// MARK: - Main View
struct StackSortViewV2: View {
    @State private var stacks: [StStV2Stack] = []
    @State private var selectedStack: Int? = nil
    @State private var phase: StStV2GamePhase = .start
    ...
    var body: some View {
        ZStack {
            ...
            switch phase {
            case .start:
                StStV2StartScreen(difficulty: difficulty) { startGame() }
            case .playing:
                StStV2PlayingScreen(
                    stacks: $stacks,
            ...
            case .won:
                StStV2WonScreen(moves: moves, difficulty: difficulty, onRestart: {
                    recordResult(true)
                    startGame()
                })
            }
    ...
    private func startGame() {
        var allDiscs: [StStV2DiscColor] = StStV2DiscColor.allCases.flatMap { Array(repeating: $0, count: 5) }
        allDiscs.shuffle()
        stacks = (0..<5).map { i in
            var s = StStV2Stack()
            if i < 3 {
...
// MARK: - Sub-Views
private struct StStV2StartScreen: View {
...
private struct StStV2WonScreen: View {
    let moves: Int
    let difficulty: Double
    let onRestart: () -> Void
    var body: some View {
...
private struct StStV2PlayingScreen: View {
    @Binding var stacks: [StStV2Stack]
        ...
        VStack(spacing: 20) {
            ...
            HStack(spacing: 10) {
                ForEach(0..<stacks.count, id: \.self) { i in
                    StStV2StackView(stack: stacks[i], isSelected: selectedStack == i)
...
private struct StStV2StackView: View {
    let stack: StStV2Stack
    let isSelected: Bool
