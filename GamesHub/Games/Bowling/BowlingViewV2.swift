...
struct BowlingViewV2: View {
    @State private var pins: [BwlV2Pin] = BowlingViewV2.makePins()
    @State private var phase: BwlV2Phase = .start
    ...
    static func makePins() -> [BwlV2Pin] {
        ...
        for (r, row) in rows.enumerated() {
            for (c, id) in row.enumerated() {
                ...
                result.append(BwlV2Pin(id: id, position: CGPoint(x: x, y: y)))
            }
        }
        return result
    }
