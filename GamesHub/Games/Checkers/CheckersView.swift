...
private class CheckersGame: ObservableObject {
    ...
    @Published var turn: CheckersPiece = .red
    ...
    private func dirs(for piece: CheckersPiece) -> [(Int, Int)] {
    ...
    private func normalMoves(from pos: CheckersPosition) -> [CheckersMove] {
        ...
        return dirs(for: piece).compactMap { (dr, dc) in
            ...
            return CheckersMove(from: pos, to: CheckersPosition(row: nr, col: nc), captured: nil)
        }
    }
...
struct CheckersView: View {
    ...
    private func glassCellView(row: Int, col: Int, size: CGFloat) -> some View {
        ...
        ZStack {
            ...
            if piece != .none {
                glassPieceView(piece: piece, size: size)
            }
        }
        .frame(width: size, height: size)
    ...
    @ViewBuilder
    private func glassPieceView(piece: CheckersPiece, size: CGFloat) -> some View {
