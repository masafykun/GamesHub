...
enum Match3V2GemColor: Int, CaseIterable {
...
struct Match3V2Gem: Identifiable, Equatable {
    let id: UUID
    var color: Match3V2GemColor

    init(color: Match3V2GemColor) {
        self.id = UUID()
        self.color = color
    }

    static func == (lhs: Match3V2Gem, rhs: Match3V2Gem) -> Bool {
        lhs.id == rhs.id
    }
}
...
class Match3V2Board: ObservableObject {
    static let size = 8

    @Published var grid: [[Match3V2Gem?]]
    @Published var score: Int = 0
    @Published var movesRemaining: Int = 30
    @Published var selectedPosition: (row: Int, col: Int)? = nil
    @Published var isGameOver: Bool = false
    @Published var sparklingIDs: Set<UUID> = []
    @Published var activeColorCount: Int = 5

    var availableColors: [Match3V2GemColor] {
        Array(Match3V2GemColor.allCases.prefix(activeColorCount))
    }

    init(colorCount: Int = 5) {
        self.activeColorCount = colorCount
        grid = Array(
            repeating: Array(repeating: nil, count: Match3V2Board.size),
            count: Match3V2Board.size
        )
        fillBoard()
        resolveInitialMatches()
    }

    func randomColor() -> Match3V2GemColor {
        availableColors.randomElement()!
    }

    func fillBoard() {
        for row in 0..<Match3V2Board.size {
            for col in 0..<Match3V2Board.size {
                if grid[row][col] == nil {
                    grid[row][col] = Match3V2Gem(color: randomColor())
                }
            }
        }
    }
    ...
    func resolveInitialMatches() {
        ...
        while iter < 100 {
            ...
            for pos in matched {
                grid[pos.row][pos.col] = Match3V2Gem(color: randomColor())
            }
            iter += 1
        }
    }
    ...
    func cascade(initial: [(row: Int, col: Int)]) {
        ...
        while !current.isEmpty {
            let ids = current.compactMap { grid[$0.row][$0.col]?.id }
            sparklingIDs.formUnion(ids)
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.45) { [weak self] in
                ...
                self.sparklingIDs.subtract(ids)
            }
            dropGems()
            fillBoard()
            current = findMatches()
    ...
    func findMatches() -> [(row: Int, col: Int)] {
        ...
        for row in 0..<Match3V2Board.size {
            var col = 0
            while col < Match3V2Board.size {
                guard let gem = grid[row][col] else { col += 1; continue }
                var len = 1
                while col + len < Match3V2Board.size,
        ...
        for col in 0..<Match3V2Board.size {
            var row = 0
            while row < Match3V2Board.size {
                guard let gem = grid[row][col] else { row += 1; continue }
                var len = 1
                while row + len < Match3V2Board.size,
    ...
    func dropGems() {
        for col in 0..<Match3V2Board.size {
            var write = Match3V2Board.size - 1
            for row in stride(from: Match3V2Board.size - 1, through: 0, by: -1) {
...
// MARK: - Gem Cell V2 (Glassmorphism)

struct Match3V2GemCell: View {
    let gem: Match3V2Gem
    let isSelected: Bool
    ...
    var body: some View {
        ...
        .scaleEffect(isSparkling ? (pulse ? 1.25 : 0.85) : (isSelected ? 1.10 : 1.0))
        .animation(
            isSparkling
                ? .easeInOut(duration: 0.18).repeatForever(autoreverses: true)
                : .spring(response: 0.28, dampingFraction: 0.58),
            value: isSparkling ? pulse : isSelected
        )
        .onChange(of: isSparkling) { _, sparkling in
            if sparkling { pulse.toggle() }
        }
        .onAppear {
            if isSparkling { pulse = true }
        }
    }
}
...
struct Match3ViewV2: View {
    @StateObject private var board = Match3V2Board(colorCount: 5)
    ...
    var gemSize: CGFloat {
        ...
        return (screen - padding * 2 - CGFloat(Match3V2Board.size + 1) * 4) / CGFloat(Match3V2Board.size)
    }
    ...
    var boardView: some View {
        VStack(spacing: 4) {
            ForEach(0..<Match3V2Board.size, id: \.self) { row in
                HStack(spacing: 4) {
                    ForEach(0..<Match3V2Board.size, id: \.self) { col in
    ...
    func cellView(row: Int, col: Int) -> some View {
        ...
        ZStack {
            ...
            if let gem = board.grid[row][col] {
                let isSparkling = board.sparklingIDs.contains(gem.id)
                Match3V2GemCell(
                    gem: gem,
