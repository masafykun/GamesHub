...
enum ReactionV2Difficulty: String {
    case easy   = "Easy"
    case medium = "Medium"
    case hard   = "Hard"
...
struct ReactionV2RoundResult: Identifiable {
    let id = UUID()
    let round: Int
...
struct ReactionViewV2: View {
    ...
    var idleView: some View {
        VStack(spacing: 28) {
            ...
            // Info card
            VStack(spacing: 14) {
                ReactionV2InfoRow(icon: "circle.fill", color: .green,  text: "Green screen = tap now!")
                ReactionV2InfoRow(icon: "exclamationmark.triangle.fill", color: .yellow, text: "Early tap = +500ms penalty")
                ReactionV2InfoRow(icon: "chart.line.uptrend.xyaxis", color: .cyan,   text: "5 rounds — lower average wins")
                ReactionV2InfoRow(icon: difficulty.icon, color: difficulty.color, text: "Difficulty adapts to your skill")
            }
            ...
            VStack(spacing: 20) {
                ...
                VStack(spacing: 8) {
                    ForEach(results) { result in
                        ReactionV2RoundRow(result: result)
                    }
                }
                .padding(.horizontal, 20)
    ...
    func handleTap() {
        ...
        case .waiting:
            ...
            waitTimer = nil
            earlyTap = true
            let result = ReactionV2RoundResult(round: currentRound, milliseconds: penaltyMs, isPenalty: true)
        ...
        case .ready:
            ...
            let result = ReactionV2RoundResult(round: currentRound, milliseconds: ms, isPenalty: false)
            results.append(result)
            phase = .tapped
...
struct ReactionV2RoundRow: View {
    let result: ReactionV2RoundResult
